...
export default function SalesInvoice({ order }) {
    //console.log(order)
    const invoiceData = ConvertIsoDateToNormal(order.createdAt)
    const subTotal = order.orderItems.reduce((acc, currentItem) => acc + currentItem.price * currentItem.quantity, 0).toFixed(2);
    const tax = 20
            ...
            <div ref={invoiceRef}>
                <div className="max-w-4xl mx-auto border border-gray-500 p-8 rounded-sm text-slate-800 dark:text-slate-200 bg-white dark:bg-slate-800">
                    ...
                    <div className="flex justify-between border-b border-gray-500 pb-8">
                        ...
                        <Image width={500} height={500} src="/applogo.png" alt="limifood logo" className="w-36 h-16" />
                    </div>
                    {/* Header End */}
